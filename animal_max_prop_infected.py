...
for i in range(0,len(newGraphList)):
        ...
        #this is all for making the plot look nice
        vecT=[i for k in range(0,100)]  #these put the uniform and degree-based measurements in different columns
        vecU=[i+0.5 for k in range(0,100)]  
        weightsT = [2*k for k in Counter(maxInfectedVecTango).values() for m in range(k)] #these increase the point size on the plot in proportion to the frequency of that value being attained
        weightsU = [2*k for k in Counter(maxInfectedVecUniform).values() for m in range(k)]
        if i ==2:
          plt.scatter(vecT,maxInfectedVecTango,c='r',label='Degree-based',s=weightsT)
          plt.scatter(vecU,maxInfectedVecUniform,c='b',label='Uniform',s=weightsU)
        else:
          plt.scatter(vecT,maxInfectedVecTango,c='r',s=weightsT)
          plt.scatter(vecU,maxInfectedVecUniform,c='b',s=weightsU)
...
plt.xlabel('')
plt.ylabel('Maximum Proportion Infected')
plt.title("Maximum proportion infected for wildlife contact networks")
plt.legend()
plt.savefig('EpidemicsAntHyenaTortoise.pdf')            
plt.show()            
