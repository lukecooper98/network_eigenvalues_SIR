...
for i in range(0,len(newGraphList)):
        ...
        weightsT = [2*k for k in Counter(maxTimeVecTango).values() for m in range(k)]
        weightsU = [2*k for k in Counter(maxTimeVecUniform).values() for m in range(k)]
        if i ==2:
          plt.scatter(vecT,maxTimeVecTango,c='r',label='Degree-based',s=weightsT)
          plt.scatter(vecU,maxTimeVecUniform,c='b',label='Uniform',s=weightsU)
        else:
          plt.scatter(vecT,maxTimeVecTango,c='r',s=weightsT)
          plt.scatter(vecU,maxTimeVecUniform,c='b',s=weightsU)
...
plt.title("Simulation Time for Wildlife Contact Networks")
plt.legend()
plt.savefig('SimTimeAntHyenaTortoise.pdf')            
plt.show()             
